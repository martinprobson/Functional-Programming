...
object List {
  ...
  //
  implicit def listEqual[A]: Eq[List[A]] =
    Eq.instance[List[A]] { (l1, l2) => equal(l1,l2) }

  //
  // Equal method
  //
  def equal[A](l1: List[A], l2: List[A]): Boolean = (l1, l2) match {
    case (Nil, Nil) => true
  ...
  def apply_TR[A](as: A*): List[A] = {
    @tailrec
    def _apply(accum: List[A],as: A*): List[A] =  
      if (as.isEmpty) accum
  ...
  */
  ...
  def hasSubsequence[A](sup: List[A], sub: List[A]): Boolean = {
    ...
    // Slice a list
    //
    def slice(l: List[A], from: Int, to: Int): List[A] = {
...
object Run extends App {
    ...
    def toStr(b: String, a: Int) = b + " " + a.toString
