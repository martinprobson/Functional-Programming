...
class Exercise10_3Test extends AnyFunSuite with Checkers {

  case class TestCase(name: String, property: Prop)

  def checkMonoidLaw(m: Monoid[Int => Int]): Prop = forAll { (a1: Int => Int) =>
    isEqual(m.op(m.zero, a1), a1) &&
  ...
  val props = Seq(
    TestCase(
      "Check Monoid Law - firstEndoMonoid",
      checkMonoidLaw(firstendoMonoid).label("firstEndoMonoid")
    ),
    TestCase(
      "Check Monoid Law - lastEndoMonoid",
      checkMonoidLaw(lastendoMonoid).label("lastEndoMonoid")
    )
  )
