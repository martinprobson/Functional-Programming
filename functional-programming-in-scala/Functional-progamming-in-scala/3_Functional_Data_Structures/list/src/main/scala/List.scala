...
object List {
  ...
  def head[A](l: List[A]): A = l match {
    case Nil =>
      throw new java.util.NoSuchElementException("head of an empty list")
    case Cons(h, _) => h
  }
  ...
  def tail[A](l: List[A]): List[A] = l match {
    case Nil =>
      throw new java.lang.UnsupportedOperationException("Tail of an empty list")
    case Cons(_, t) => t
  }
  ...
  def hasSubsequence[A](sup: List[A], sub: List[A]): Boolean = {
    ...
    if ((sub == Nil) || (sup == sub))
      ...
      if (sl == Nil)
        false
      else if (slice(sl, 1, List.length(sub)) == sub)
        true
      else
  ...
  def addLists(l1: List[Int], l2: List[Int]): List[Int] = {
    @tailrec
    def _addLists(l1: List[Int], l2: List[Int], accum: List[Int]): List[Int] =
      (l1, l2) match {
        case (Nil, Nil) => Nil
        case (Nil, l2)  => l2
        case (l1, Nil)  => l1
        case (Cons(h1, Nil), Cons(h2, t2)) =>
          List.append(accum, Cons(h1 + h2, t2))
        case (Cons(h1, t1), Cons(h2, Nil)) =>
          List.append(accum, Cons(h1 + h2, t1))
        case (Cons(h1, t1), Cons(h2, t2)) =>
          _addLists(t1, t2, List.append(accum, List(h1 + h2)))
      }
    _addLists(l1, l2, Nil)
  }
  ...
  def zipWith[A](l1: List[A], l2: List[A])(f: (A, A) => A): List[A] = {
    @tailrec
    def _zipWith(l1: List[A], l2: List[A], accum: List[A])(
        f: (A, A) => A
    ): List[A] = (l1, l2) match {
      case (Nil, Nil) => Nil
      case (Nil, l2)  => l2
      case (l1, Nil)  => l1
      case (Cons(h1, Nil), Cons(h2, t2)) =>
        List.append(accum, Cons(f(h1, h2), t2))
      case (Cons(h1, t1), Cons(h2, Nil)) =>
        List.append(accum, Cons(f(h1, h2), t1))
      case (Cons(h1, t1), Cons(h2, t2)) =>
        _zipWith(t1, t2, List.append(accum, List(f(h1, h2))))(f)
    }
