...
class BasicParTest extends FixtureAnyFlatSpec {
  ...
  def withFixture(test: OneArgTest): Outcome = {
    ...
    val es = Executors.newThreadPerTaskExecutor(threadFactory)
  ...
  "A call to lazyUnit" should "simply return its parameter when called with an ExecutorService and get" in {
    es =>
      val res = lazyUnit(1)(es).get
      assert(res === 1)
  }
  it should "return the same value as if unit were used" in { es =>
  ...
  "A sum on Vector(1,2)" should "return 3" in { es =>
    val res = sum(Vector(1, 2))(es).get
    assert(res === 3)
  }
  it should "be the same result with map2 expanded" in { es =>
    val orig = sum(Vector(1, 2))(es).get
    val withMap2 = map2(fork(unit(1)), fork(unit(2)))(_ + _)(es).get
    assert(withMap2 === orig)
  }
  it should "be the same result with map2/unit expanded" in { es =>
    val orig = sum(Vector(1, 2))(es).get
    val withUnit =
      map2(fork(_ => UnitFuture(1)), fork(_ => UnitFuture(2)))(_ + _)(es).get
    assert(withUnit === orig)
  }
  it should "be the same result with map2/unit/fork expanded" in { es =>
    val orig = sum(Vector(1, 2))(es).get
    val full = map2(
      (es: ExecutorService) =>
        es.submit(new Callable[Int] {
          def call: Int = ((es: ExecutorService) => UnitFuture(1))(es).get
        }),
      fork((es: ExecutorService) => UnitFuture(2))
    )(_ + _)(es).get
    assert(full === orig)
  }

  "A sum on Vector(1,2,3,4)" should "return 10" in { es =>
    val res = fork(sum(Vector(1, 2, 3, 4)))(es).get
    assert(res === 10)
  }
  it should "be the same result when the call sequence is expanded" in { es =>
    ...
    val expand = map2(
      ...
      map2(
        (es: ExecutorService) =>
          es.submit(new Callable[Int] {
            def call: Int = ((es: ExecutorService) => UnitFuture(3))(es).get
          }),
        fork((es: ExecutorService) => UnitFuture(4))
      )(_ + _)
    )(_ + _)(es).get
    assert(orig === expand)
  }

  "Exercise 7.4 asyncF test" should "convert any function to run asynchronously and return the same result" in {
    es =>
      // def sumSeq(ints: Int*) = (0 /: ints){_+_}  // Define a simple function for our test
      def sumSeq(ints: Int*) = ints.sum // Define a simple function for our test
  ...
  "Execise 7.5 parMap" should "operate correctly with sequence" in { es =>
  ...
  "A max on Vector(-10,0,999,1,2,3,4)" should "return 999" in { es =>
