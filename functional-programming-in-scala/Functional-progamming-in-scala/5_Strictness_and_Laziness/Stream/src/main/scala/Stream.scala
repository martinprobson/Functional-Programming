...
sealed trait Stream[+A] {
  ...
  def take_UF(n: Int): Stream[A] =
    Stream.unfold((this, n)) { case (s, n) =>
      s match {
        case Cons(h, tl) if n > 0 => Some((h(), (tl(), n - 1)))
        case _                    => None
      }
    }
  ...
  def map_UF[B](f: (A) => B): Stream[B] =
    Stream.unfold((this, Empty: Stream[B])) { case (a, b) =>
      a match {
        case Cons(h, tl) => Some(f(h()), (tl(), b))
        case _           => None
      }
    }
...
//                           .filter(filter50)
//
// Note: Only the take_NTR version seems to behave and evaluate nothing in the statement below,
// replacing the implementation of take with take_TR always evaluates the first member of the stream.....
  val s = Stream(1, 2, 3, 4).map(add10).take(2)
  println(s)
//    trace("TOLIST")
  // s.toList
